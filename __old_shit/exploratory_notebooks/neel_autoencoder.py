...
def get_recons_loss(num_batches=5, local_encoder=None):
    ...
    for i in range(num_batches):
        ...
        recons_loss = model.run_with_hooks(
            ...
            fwd_hooks=[
                (
                    ...
                    partial(replacement_hook, encoder=local_encoder),
                )
            ],
        )
        ...
        zero_abl_loss = model.run_with_hooks(
            tokens,
            return_type="loss",
            fwd_hooks=[(utils.get_act_name("post", 0), zero_ablate_hook)],
        )
    ...
    print(
        f"loss: {loss:.4f}, recons_loss: {recons_loss:.4f}, zero_abl_loss: {zero_abl_loss:.4f}"
    )
...
def basic_feature_vis(text, feature_index, max_val=0):
    ...
    _, cache = model.run_with_cache(
        text, stop_at_layer=1, names_filter=utils.get_act_name("post", 0)
    )
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                out = gr.HTML(
                    label="Neuron Acts",
                    value=basic_feature_vis(starting_text, feature_id),
                )
        for inp in inputs:
