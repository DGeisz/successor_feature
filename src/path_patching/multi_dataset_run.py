...
from src.dataset.clean_dataset import CleanDataset, get_clean_dataset
from src.dataset.dataset import get_normalizing_function_for_activations
...
class MultiDatasetRun:
    def __init__(self, N: int, target_nodes: List[AttnHead], target_location: str):
        self.N = N

        self.clean_dataset = get_clean_dataset(N)
        self.corrupted_datasets = get_corrupted_datasets(N)
        ...
        normalizing_function = get_normalizing_function_for_activations(
